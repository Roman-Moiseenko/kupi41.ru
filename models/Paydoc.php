...
class Paydoc extends ModelTable
{
    const PAY_NOPAY = 0;
    const PAY_INVOICE = 1;
    const PAY_ECHECK = 2;

    static public $caption = array(self::PAY_INVOICE => 'счет', self::PAY_ECHECK => 'е-чек');
    public $safe = array('id', 'productorder', 'user', 'datepaydoc', 'numberpaydoc', 'typepaydoc'); //id === order_id
    const TABLE_NAME = 'paydoc';
    private $pay = null;

    public function SaveAndPayOrder(Order &$order)
    {
        $this->pay->PayOrder($order);
    ...
    {
        //TODO Определяем тип платежа и создаем объект данного типа, по умолчанию - БЕЗ_ОПЛАТЫ
        parent::__construct();
        $typepaydoc = ($typepaydoc == null) ? 0: $typepaydoc;
        $this->typepaydoc = $typepaydoc;
        switch ($typepaydoc) {
            case (self::PAY_NOPAY):
                $this->pay = new PayNot();
                break;
            case (self::PAY_INVOICE):
                $this->pay = new PayInvoice();
                break;
            case (self::PAY_ECHECK):
                $this->pay = new PayNow();
                break;
        }
    }
    ...
    static public function _models($user)
    {
        ...
        $items = App::gI()->db->query('SELECT * FROM '.$modelname::TABLE_NAME . ' WHERE user = '.$user);
        foreach ($items as $item) {
            $typepaydoc = (count($item) != 0) ? $item['typepaydoc'] : null;
            $model = new $modelname($typepaydoc);
            $model->__attributes = $item;
            $results[] = $model;
        }
        return $results;
    }
    static public function model($idorder) //Выборка по id Order!!!
    {
        $modelname = get_called_class();
        $item = App::gi()->db->query('SELECT * FROM '.$modelname::TABLE_NAME.' WHERE productorder = '. $idorder);
        $typepaydoc = (count($item) != 0) ? $item[0]['typepaydoc'] : null;
        $model = new $modelname($typepaydoc);
        //print_r($item[0]);
...
//    private
    ...
    public function loadPaydoc()

    {
        $this->pay->loadPaydoc($this->__attributes);
        //TODO Создание счета  или электронного чека  -- Перегруженная функция
       /* if ($this->typepaydoc == self::PAY_INVOICE) {
            //СЧЕТ
        ...
        if ($this->typepaydoc == self::PAY_ECHECK) {
            //Е-чек
            $filename = App::gI()->config->echeck . $this->id . '.pdf';
    ...
    //TODO Удалить, т.к. вынесена в class PayInvoice!!
    private function createXLS()
    {
        ...
        /** !!!!!!!!!!!! */
        $sheet->setTitle('Счет N ' . $this->numberpaydoc);
        ...
        /** !!!!!!!!!!!! */
        $sheet->setCellValue('B11','Счет на оплату № ' . $this->numberpaydoc . ' от ' . _f::dateToHTML($this->datepaydoc));
        ...
        $row = 21; $i = 0;
        /** !!!!!!!!!!!! */
        $order = Order::model($this->productorder);
        ...
        /** !!!!!!!!!!!! */
        header('Content-Disposition:attachment;filename="Счет на оплату ' . $this->numberpaydoc . '.xlsx"');
